...
def mae_with_std_dev_penalty_np(y_true, y_pred, std_weight=1.0, de_weight=1.0, clip_weight_std=None, clip_weight_dev=None):
    """
    MAE + std penalty + shape deviation penalty.
    ...
    std_weight : float
        Weight for std penalty.
    de_weight : float
        Weight for deviation penalty.
    clip_weight_std : float or None
        Optional max clip for std penalty weight.
    clip_weight_dev : float or None
        Optional max clip for deviation penalty weight.

    Returns:
    -------
    total_loss : float
        Combined loss value (mean over all points).
    """
