# expanding_window_reverse_weekly_train.py
# author: Yajing Liu
# date: 2025-06-08
...
@click.option('--results', type=str, default="./results/tables/sarima/expanding_window_reverse_weekly_predictions.csv", help="Path to save results CSV")
def main(data, results):
    """
    Performs SARIMA reverse expanding window forecasting (weekly) and saves evaluation metrics.
    """
    ...
    # Reverse expanding window setup
    window_step = 96 * 7  # 1 week
    fh = list(range(1, 96 + 1))  # 1 day forecast horizon
    ...
    # Reverse expanding window
    train_end = test_start  # right before test
    ...
    # Run reverse expanding window CV
