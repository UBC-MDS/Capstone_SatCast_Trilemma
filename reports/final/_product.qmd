...
We included HWES and SARIMA as simple, fast-to-train baselines for coarse-grained fee trend forecasting. These models approximated day-level seasonal drifts but failed to capture sharp intraday dips and spikes (see @fig-hwes-sarima). Although their predictions remained stable, they performed poorly on short-term volatility metrics. HWES yielded a custom loss of `{python} f"{metrics_df.loc['custom_loss', 'HWES']:.2f}"` and an RMSE of `{python} f"{metrics_df.loc['rmse', 'HWES']:.2f}"`. SARIMA produced similar results, with a custom loss of `{python} f"{metrics_df.loc['custom_loss', 'SARIMA']:.2f}"` and an RMSE of `{python} f"{metrics_df.loc['rmse', 'SARIMA']:.2f}"`. These findings highlight the limitations of traditional models in capturing high-frequency dynamics in transaction fee patterns. Both models performed no better than the global median baseline, which yielded a custom loss of `{python} f"{metrics_df.loc['custom_loss', 'Median']:.2f}"` and an RMSE of `{python} f"{metrics_df.loc['rmse', 'Median']:.2f}"`.
```{python}
...
We chose Prophet for its ease of use, interpretability, and built-in support for trend shifts, seasonality, and holiday effects—features well-suited to capturing macro-level fee patterns with minimal tuning. Its flexible trend components and seasonal priors improved global fits. However, Prophet still oversmoothed transient spikes. It recorded a custom loss of `{python} f"{metrics_df.loc['custom_loss', 'Prophet']:.2f}"` and an RMSE of `{python} f"{metrics_df.loc['rmse', 'Prophet']:.2f}"`.

We selected XGBoost for its ability to model nonlinear interactions between engineered features such as lagged fees and mempool congestion signals, while remaining efficient to train and tune. XGBoost outperformed all classical models, achieving a custom loss of `{python} f"{metrics_df.loc['custom_loss', 'XGBoost']:.2f}"` and an RMSE of `{python} f"{metrics_df.loc['rmse', 'XGBoost']:.2f}"`. Despite this, it tended to underestimate sharp fee jumps and often produced flat or conservative predictions in highly volatile regions (see @fig-prophet-xgboost).
...
The neural models marked a meaningful shift in modeling capacity, enabling the system to learn from richer temporal patterns and nonlinear interactions. We selected DeepAR for its ability to capture sequential dependencies through autoregressive recurrence. It offered a pathway toward future probabilistic forecasting and achieved a custom loss of `{python} f"{metrics_df.loc['custom_loss', 'DeepAR']:.2f}"` and an RMSE of `{python} f"{metrics_df.loc['rmse', 'DeepAR']:.2f}"`.
